...
export const CompareRow = memo(function CompareRow({
  ...
  // Nothing has happened in this column yet. Splitting the bar evenly was the
  // old answer to the divide-by-zero and it invents a contest: at full time a
  // "0 red cards each" row drew half the bar in one kit and half in the other,
  // which reads as data. An empty track says what is true — no fouls, no
  // cards, no shots — and says it the same way live at 0-0.
  const nothingYet = total <= 0;
  const homeShare = nothingYet ? 0 : homeValue / total;
  ...
  return (
    <div className={cn('py-2', className)}>
      ...
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={100}
        aria-valuenow={nothingYet ? 0 : Math.round(homeShare * 100)}
