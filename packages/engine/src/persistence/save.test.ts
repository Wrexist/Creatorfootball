...
const stripToV1 = (state: GameState): Record<string, unknown> => {
  const { boardPressure: _dropped, ...rest } = state;
...
describe('save migrations', () => {
  ...
  it('migrates a pre-board save forward with the field defaulted in', () => {
  ...
  it('leaves everything else about the save untouched', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    // Everything except the newly added field must be equal; object spread
