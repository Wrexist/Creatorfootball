...
The qualification record below describes the earlier local pass. On September 24, signed iOS `1.0.16 (10)` uploaded successfully and Apple began processing it; signed Android `1.0.16 (4)` became active on the internal track. These builds restore the original CF crest on top of the qualified gameplay repairs. RevenueCat store credentials and both-platform product/entitlement/offering wiring were rechecked earlier that day. The latest iOS workflow passed lint, typecheck, 998 tests, production build and browser flows before archive/upload. See [STORE_SETUP_STATUS.md](STORE_SETUP_STATUS.md) for current processing/build-selection status, artifact hashes, listing evidence and unresolved gates, and [NATIVE_TEST_HANDOFF.md](NATIVE_TEST_HANDOFF.md) for remaining device/commerce checks.
