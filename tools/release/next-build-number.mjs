...
async function main() {
  ...
  if (chosen == null) {
    if (has('--require-asc')) {
      throw new Error('A release requires a successful App Store Connect build-number lookup. No fallback number was emitted.');
    }
...
main().catch((e) => {
  if (has('--require-asc')) {
    log(e?.message || 'App Store Connect build-number resolution failed.');
    process.exitCode = 1;
    return;
  }
